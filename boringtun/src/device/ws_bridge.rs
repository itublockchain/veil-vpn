...
pub fn run_ws_bridge(
    ...
    let udp = match UdpSocket::bind(format!("127.0.0.1:{}", local_udp_port)) {
        Ok(s) => s,
        Err(e) => {
            tracing::error!("WS bridge: failed to bind UDP on port {}: {}", local_udp_port, e);
            return;
        }
    };
    ...
    while !shutdown_flag.load(Ordering::Relaxed) {
        ...
        let mut ws = match connect_ws(ws_url) {
            Some(ws) => ws,
            None => {
                tracing::warn!("WS bridge: connection failed, retrying in {}ms", RECONNECT_DELAY_MS);
